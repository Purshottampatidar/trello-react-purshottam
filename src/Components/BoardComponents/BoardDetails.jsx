...
const BoardDetails = () => {
  ...
  return (
    <Box h={'90vh'} overflow={'hidden'}>{loading ?(
      <Box fontSize={'5rem'} textAlign={'center'}mt={'20%'} fontStyle={'bold'}>Loading...</Box>
    ): error ? (
      <Box>No Result found...</Box>
    ):(
      <Flex bg={'#0079BF'}
        style={{
          background : board && board.prefs && board.prefs.backgroundImage ? `url(${board.prefs.backgroundImage}) no-repeat center/cover` : '',
        }}
      >
        <Slider />
        <Flex  w={'90vw'} direction={'column'} >
          <Box  display={'flex'} bg={'gray.300'}  h={'5vh'} alignItems={'center'} pl={'1rem'} fontSize={'1.2rem'} fontWeight={'bold'}>
            {board && board.name}
          </Box>
          <Box display={'flex'} h={'85vh'} overflowX={'scroll'} gap={'2rem'} p={3}    > 
